...
def soldier_page():
    ...
    with st.form("constraints_form"):
        ...
        request_type = st.selectbox("סוג אילוץ / תבנית מבוקשת:", ["יומי", "שבוע-שבוע", "חמשו\"ש", "יומיים", "חופשה מיוחדת / אירוע אישי"])
...
def commander_page():
    ...
    if run_optimization or 'current_df' in st.session_state:
        # יצירת הנתונים בפעם הראשונה בלבד
        if run_optimization or 'current_df' not in st.session_state:
            with st.spinner("מנוע ה-CP-SAT מנתח את אילוצי הסד\"כ וטווחי התאריכים..."):
                ...
                # יצירת סטטוסים ראשוניים מגוונים כדי לאפשר חילופים
        ...
        # הגדרת תיבת הבחירה בתוך תאי הטבלה לאפשרויות שביקשת
        status_options = ["נוכח בבסיס", "בבית (חופשה)", "יציאה קצרה (כמה שעות)", "הארכת שהות (גיבוי)"]
        
        # הצגת עורך הטבלה האינטראקטיבי
        ...
        for idx, day in enumerate(days_cols):
            with calc_cols[idx]:
                # סכימת כמות הלוחמים שנמצאים פיזית בבסיס באותו יום (נוכחים או מוארכי שהות)
